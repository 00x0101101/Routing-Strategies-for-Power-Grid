function [ data, I_real, U_real, P_load, P_generate] = Limited_LongestPath_loss2(Connect, I_real, U_real, P_load, P_demand, ...
...
%         aa = find(compare_source(source) <= Capacity_source(source)*0.001);
        aa = find(abs(P_generate(source)) <= Capacity_source(source)*0.001);
        source_temp(aa) = -1;
...
%                 [ p_in, p_out ] = Power_flow( select_source, I_real_temp2, U_real_temp2, U_rated );
%                 P_present = p_out/eff - p_in;
%                 P_generate(select_source) = P_present + Capacity_source(select_source);
