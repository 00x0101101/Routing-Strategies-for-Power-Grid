function [ data, Status_after, status_after ] = CapLimited( Map, source, destination, Status_before, Capacity, demand )
    Map_temp = Map;
    demand_temp = demand;
...
%         [ distance_temp, route_temp ] = Dijk( Map_temp, source, destination );
        [ distance_temp, route_temp ] = ShortestPath( Map_temp, source, destination );
        if distance_temp == Inf
            flag = false;
