function [ data, Status_after, status_after, source_status_after ] = SourceLimited( Map, source, destination, Status_before, Capacity, demand, source_status_before, source_Cap )
    Map_temp = Map;
    demand_temp = demand;
...
%             x = find(compare_source_status == b);
            demand_temp2 = demand_temp + compare_source_status(sa);
            % judge capacity limitation
            status_temp = status;
            temp=[];
            for i=1:1:length(route_temp)-1
