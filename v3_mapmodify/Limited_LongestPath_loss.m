function [ data, I_real, U_real, P_load, P_out] = Limited_LongestPath_loss(Connect, I_real, U_real, P_load, P_demand, destination, U_rated, source, Capacity_source, Capacity_line, R, P_out)
    ...
    while flag
        ...
        [ Map_temp ] = Map_cal_longest( Map_temp, I_real_temp2, I0_temp, R ); %
        % modify source
...
%         aa = find(abs(compare_source(source)) <= Capacity_source(source)*0.001);
        ...
        if length(tt)>1 % 超过一条最短路径，判断真实损耗
            for k = 1:1:length(tt)
                ...
                [ I_real_temp3, U_real_temp3 ] = real_IU_calculate( route_temp, I_real_temp2, U_real_temp2, P_load, U_rated, R, P_out );
...
%             capacity_trans_temp = I0_temp;
            ...
            [ I_real_temp1, U_real_temp1 ] = real_IU_calculate( route_temp, I_real_temp2, U_real_temp2, P_load, U_rated, R, P_out );
        else
            index = tt;
%             capacity_trans_temp = I0_temp;
            distance_temp = distance_temp_diffsource(index);
...
%         b = Capacity_source(select_source) - Status_source_temp1(select_source);
        ...
        if b >= 0 % source
            ...
            if a >= 0 % line
                ...
                [ I_real_temp1, U_real_temp1 ] = real_IU_calculate_FtoS( route_temp_toStart, I_real_temp1, U_real_temp1, P_load, U_rated, R, P_out );
                [ I_real_temp1, U_real_temp1 ] = real_IU_calculate_StoF( route_temp_toFinish, I_real_temp1, U_real_temp1, P_load, U_rated, R, P_out );
                I_real_temp2 = I_real_temp1;
                U_real_temp2 = U_real_temp1;
...
%             U_real_temp1(route_temp(2),select_source) = U_real_temp1(select_source,route_temp(2));
            ...
            [ I_real_temp1, U_real_temp1 ] = real_IU_calculate_StoF( route_temp, I_real_temp1, U_real_temp1, P_load, U_rated, R, P_out );
